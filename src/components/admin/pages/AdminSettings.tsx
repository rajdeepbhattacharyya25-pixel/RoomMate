...
export const AdminSettings: React.FC<AdminSettingsProps> = ({
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-5xl">
      ...
      {/* Platform Identity & Release Header */}
      <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <img
            src="/logo.png"
            alt="RoomMate"
            className="w-14 h-14 rounded-2xl object-contain shadow-xs border border-slate-100 p-0.5 bg-slate-50 shrink-0"
          />
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-bold text-slate-900 tracking-tight">RoomMate Campus Financial Precision</h2>
              <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 uppercase tracking-wider">
                Production Core
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 text-xs bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200/80">
          ...
          <span className="font-semibold text-slate-600">PostgreSQL Cloud Live</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6 text-xs">
        ...
        <div className="p-6 rounded-2xl border-2 border-rose-200 bg-rose-50/30 space-y-5">
          ...
          {onPurgeDemoData && (
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl bg-white border border-rose-200 shadow-sm">
              <div>
                <span className="font-bold text-slate-900 text-xs">Clear Local Cache & Re-Sync Cloud</span>
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Clears stale browser cache and triggers a fresh synchronization from Supabase Cloud. Real user data in the cloud is unaffected.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setIsPurgeModalOpen(true)}
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-rose-700 font-bold rounded-xl text-xs border border-rose-200 transition-colors"
              >
                Clear Cache & Re-Sync
              </button>
            </div>
      ...
      {/* Staging Data Cache Clear Dialog */}
      {onPurgeDemoData && (
        <ConfirmationDialog
          isOpen={isPurgeModalOpen}
          title="Clear Local Cache & Re-Sync Cloud?"
          message="This will clear cached client state from local storage and reload the dashboard with live data directly from Supabase Cloud. Real data in the database will not be affected."
          confirmText="Yes, Clear & Sync"
